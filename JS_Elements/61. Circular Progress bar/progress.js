...
((progressJs) => {
    ...
    const Progress = function (target, opts) {
        ...
        this.options = {
            ...
            prefix:      (typeof(opts.prefix) !== 'undefined') ? opts.prefix : _defaults.prefix,
    ...
    Progress.prototype.animateCount = function() {
        ...
        const update = () => {
            ...
            this._labelText.innerText = this.options.prefix + Math.round(currentValue) + this.options.suffix;

            if ((totalChange > 0 && currentValue < this._value) || (totalChange < 0 && currentValue > this._value)) {
                step++;
                setTimeout(update, stepDuration);
            } else {
                this._labelText.innerText = this.options.prefix + this._value + this.options.suffix;
            }
        };
